...
export default function Dropdown<TFieldValues extends FieldValues>({label, id, name, options, value = '', required, register, error, disabled = false}: InputProps<TFieldValues>) {
  ...
  return (
    <div className="w-full mb-5">
      ...
      <select id={id} role='combobox' defaultValue={value} {...register(name as Path<TFieldValues>, { required })} className={disabledClasses + " w-full py-3 px-6 outline-none rounded-md border-2 focus:shadow-md"}>
